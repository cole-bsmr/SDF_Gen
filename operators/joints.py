...
class MoveJoint(bpy.types.Operator):
    bl_idname = "object.move_joint"
    bl_label = "Adjust the joint location and rotation."
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def modal(self, context, event):
        ...
        if event.type in {'ESC'}:
            print("Cancelled")
            self.enable_constraints(context, True)
    ...
    def invoke(self, context, event):
        if not self.__class__.is_running:
            ...
            self.enable_constraints(context, False)
            # self.constraint_influence(context, 0.0)

            # Disable bone constraints
            self.enable_bone_constraints(context, False)

            # Unlink bones
            self.unparent_bones(context)
    ...
    def enable_constraints(self, context, enable_childof):
    ...
    def constraint_influence(self, context, influence_value):
        # Turns influence of all child of constraints to 0.0.
        for obj in context.scene.objects:
            for constraint in obj.constraints:
                if constraint.type == 'CHILD_OF':
                    constraint.influence = influence_value

    def enable_bone_constraints(self, context, enable_bone_constraints):
        for pose_bone in context.object.pose.bones:
            for constraint in pose_bone.constraints:
                constraint.enabled = enable_bone_constraints
        print("")

    def bone_to_pose(self, context):
        # bpy.ops.object.mode_set(mode='EDIT')
        armature = context.object
        for pose_bone in armature.pose.bones:
            bpy.ops.pose.armature_apply(selected=False)

    def unparent_bones(self, context):
        print("SOMETHING")
        bpy.ops.object.mode_set(mode='EDIT')
        for bone in bpy.context.object.data.edit_bones:
            self.parent_data[bone.name] = (bone.parent.name if bone.parent else None, bone.use_connect)
            bone.parent = None
        bpy.ops.object.mode_set(mode='POSE')

    def restore_bone_parents(self, context):
        ...
        for pose_bone in context.object.pose.bones:
            ...
            # Restore parent
            if pose_bone.name in self.parent_data:
                parent_name, use_connect = self.parent_data[pose_bone.name]
                edit_bone = context.object.data.edit_bones[pose_bone.name]
                if parent_name:
                    edit_bone.parent = context.object.data.edit_bones[parent_name]
    ...
    def finish(self, context):
        ...
        self.bone_to_pose(context)
        self.restore_bone_parents(context)
        self.enable_constraints(context, True)
        # self.constraint_influence(context, 1.0)
        self.enable_bone_constraints(context, True)
        set_inverse(self, context)
        self.__class__.is_running = False
